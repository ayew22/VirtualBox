...
import com.zb.vv.client.stub.ContentProviderProxy;
import com.zb.vv.client.stub.ShadowPendingActivity;
import com.zb.vv.client.stub.ShadowPendingReceiver;
import com.zb.vv.client.stub.ShadowPendingService;
...
import static com.fun.vbox.client.env.SpecialComponentList.protectAction;
...
public class ComponentUtils {
    ...
    public static String getComponentAction(String packageName, String name) {
        return String.format("_VBOX_%s_%s", packageName, name);
    }
    ...
    public static Intent redirectBroadcastIntent(Intent intent, int userId) {
        ...
        newIntent.putExtra("_VBOX_|_data_", data);
        return newIntent;
    }
    ...
    public static Intent redirectIntentSender(int type, String creator, Intent intent) {
        ...
        String packageName32bit = VCore.getConfig().getHostPackageName();
        switch (type) {
        ...
        selector.putExtra("_VBOX_|_intent_", intent);
        selector.putExtra("_VBOX_|_userId_", VUserHandle.myUserId());
    ...
    private static Uri processOutsideUri(int userId, boolean is64bit, Uri uri) {
        ...
        ProviderInfo info = VCore.get().getUnHookPackageManager().resolveContentProvider(authority, 0);
        if (info == null) {
            return uri;
        }
